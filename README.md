...
경기도 버스도착정보 API와 정류소 조회 API를 사용해 **정류장 단위로 버스 도착정보를 90초 간격으로 갱신**하는 HACS 커스텀 통합입니다.
...
- 등록 후:
  - 정류장 장치(device) 아래에 버스 노선별 하위 기기(device)를 만들고, 각 버스 기기마다 항목별 엔티티 생성(도착예정/남은정류장/운행상태는 sensor, 저상여부는 binary_sensor)
  - 옵션(톱니바퀴)에서 노선 추가/삭제 가능
- API 호출 제한(일 1,000회)을 고려해 **정류장당 1회/90초** 폴링
