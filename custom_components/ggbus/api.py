...
ARRIVAL_BASES = (
    ...
    "http://apis.data.go.kr/6410000/busarrivalservice/v2",
...
STATION_BASES = (
    ...
    "http://apis.data.go.kr/6410000/busstationservice/v2",
...
class GGBusApi:
    ...
    async def resolve_station_by_code(self, station_code: str) -> Station:
        ...
        payload = await self._request_with_fallback(
            [f"{base}/{path}" for base in STATION_BASES for path in ("getBusStationList", "getBusStationListv2")],
            params_candidates,
        )
    ...
    async def get_station_arrivals(self, station_id: str) -> dict[str, Arrival]:
        ...
        payload = await self._request_with_fallback(
            [f"{base}/{path}" for base in ARRIVAL_BASES for path in ("getBusArrivalListv2", "getBusArrivalList")],
            params_candidates,
        )
